...
templateUrl: './pricingUnit-details.component.html',
styleUrls: ['./pricingUnit-details.component.css']
})
export class PricingUnitDetailsComponent implements OnInit {
  formData: any;
  searchData: any = {};
  formErrors: any = {};
  ...
  getData(projectId: any, propertyId: any) {
    let fd = new FormData();
    // let obj = {};
    ...
    this.http.post(`${environment.apiHost}/AjmanLandProperty/index.php/Tathmeen/getUnitMinTathmeenPrice`, fd)
      .subscribe((data: any) => {
        console.log("Res ....");
        console.log(data);
        this.pageData = data;
  ...
  searchResourceData(data: any, searchApartmentId: any) {
    if (data.searchProjectId) {
      this.router.navigate(['pricingUnit/profile/' + data.searchProjectId + "/" + searchApartmentId.model + "/view"]);
